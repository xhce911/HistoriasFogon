...
(function(window, document, undefined) {
  ...
  const DEFAULT_SKROLLRBODY = "skrollr-body";
  ...
  const rxCamelCaseFn = function(str, letter) {
  ...
  const detectCSSPrefix = function() {
   ...
   */
  const _updateDependentKeyFrames = function() {
    const viewportHeight = documentElement.clientHeight;
   ...
   */
  const _calcSteps = function(fakeFrame, actualFrame) {
  ...
  var _parseProps = function(skrollable) {
    ...
    let keyFrameIndex = 0;
    const keyFramesLength = skrollable.keyFrames.length;

    for (; keyFrameIndex < keyFramesLength; keyFrameIndex++) {
      const frame = skrollable.keyFrames[keyFrameIndex];
      var easing;
      var value;
      var prop;
      const props = {};
  ...
  var _detect3DTransforms = function() {
    ...
    const computedTransformWithPrefix = computedStyle.getPropertyValue(
  ...
  const _removeEvent = (skrollr.removeEvent = function(
      element,
      names,
      callback
  ) {
